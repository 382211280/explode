 ...
 * @LastEditTime: 2022-07-09 23:24:19
...
void read_cb(struct bufferevent *pbuffer, void *data)
{
    ...
    s8* sys_file = "/proc/sys/kernel/version";
    ...
    s8 sys_name[30];
    read(fp,(void *)buff,30);
    sys_name[strlen(sys_name)-1] = '\0';
    printf("recv:%s\n", buff);
    if (strstr(buff, "quit"))
    {
        s8 quit_info[256] = {0};
        snprintf(quit_info,"%s will close connect,system info:%s\nquit time:%s \n",);
